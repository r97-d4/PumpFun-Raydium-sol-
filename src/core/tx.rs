...
pub async fn jito_confirm(
    ...
    logger.log(
        format!("交易耗时({}): {:?}", bundle_id, start_time.elapsed())
            .yellow()
            .to_string(),
    );
...
pub async fn new_signed_and_send_zeroslot(
    ...
    let sig = match zeroslot_client.send_transaction(&txn).await {
        Ok(signature) => signature,
        Err(_) => {
            return Err(anyhow::anyhow!("发送交易状态获取超时"
                .red()
                .italic()
                .to_string()));
        }
    ...
    txs.push(sig.clone().to_string());
    logger.log(
        format!("[交易耗时]: {:?}", start_time.elapsed())
            .yellow()
            .to_string(),
    );
...
pub async fn new_signed_and_send_nozomi(
    ...
    txs.push(sig.clone().to_string());
    logger.log(
        format!("[交易耗时(NOZOMI)]: {:?}", start_time.elapsed())
            .yellow()
            .to_string(),
    );
...
pub async fn new_signed_and_send_spam(
    ...
    for result in results {
        match result {
            ...
            Err(e) => errors.push(format!("任务失败: {:?}", e)), // Collect task failure
        }
    }
    ...
    if !errors.is_empty() {
        ...
        if successful_results.is_empty() {
            return Err(anyhow::anyhow!(format!("所有任务都失败了，错误: {:?}", errors)));
        }
    }
