...
pub async fn get_tip_value() -> Result<f64> {
    ...
    if let Ok(tip_value) = std::env::var("JITO_TIP_VALUE") {
        match f64::from_str(&tip_value) {
            ...
            Err(_) => {
                println!(
                    "环境变量JITO_TIP_VALUE无效: '{}'. 回退到百分位计算。",
                    tip_value
                );
                Err(anyhow!("环境变量TIP_VALUE无效"))
            }
        }
    } else {
        Err(anyhow!("环境变量JITO_TIP_VALUE未设置"))
    }
}
...
pub async fn get_priority_fee() -> Result<f64> {
    ...
    if let Ok(priority_fee) = std::env::var("JITO_PRIORITY_FEE") {
        match f64::from_str(&priority_fee) {
            ...
            Err(_) => {
                println!(
                    "环境变量JITO_PRIORITY_FEE无效: '{}'. 回退到百分位计算。",
                    priority_fee
                );
                Err(anyhow!("环境变量JITO_PRIORITY_FEE无效"))
            }
        }
    } else {
        Err(anyhow!("环境变量JITO_PRIORITY_FEE未设置"))
    }
}
    ...
    Fut: Future<Output = Result<Vec<Value>>>,
{
    ...
    loop {
        ...
        if let Some(status) = statuses.first() {
            ...
            progress_bar.set_message(format!("正在最终确认包 {}: {}", bundle_id, "None"));
        }

        // check loop exceeded 1 minute,
        if start_time.elapsed() > timeout {
            println!("循环超时 {:?}, 退出。", timeout);
...
impl JitoClient {
    ...
    pub async fn send_transaction(
        ...
        Err(ClientError::Other("超出最大重试次数".to_string()))
    }
