...
impl Config {
    pub async fn new() -> &'static Mutex<Config> {
        GLOBAL_CONFIG
            ...
            let logger = Logger::new("[初始化] => ".blue().bold().to_string());
...
pub const HELIUS_PROXY: &str =
    "F7pXcmPnWkamhb3WGnfqFnYPJR71ufRGAGmHQ";
...
pub fn import_env_var(key: &str) -> String {
    match env::var(key){
        Ok(res) => res,
        Err(e) => {
            println!("{}", format!("环境变量错误 {}: {}", key, e).red().to_string());
            loop{}
        }
    }
}
...
pub fn import_wallet() -> Result<Arc<Keypair>> {
    ...
    if priv_key.len() < 85 {
                    println!("{}", format!("请检查钱包私钥: 无效长度 => {}", priv_key.len()).red().to_string());
        loop{}
    }
